...
// CHECK CUSTOMER SESSION AND HANDLE FULLNAME DATA

// AJAX action to save full name
function wp2023_check_active_session($full_name) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'wp2023_customers';
    // Logic to check for an active session with the same full name
...
function wp2023_save_full_name() {
    ...
    $user_id = get_current_user_id(); // Assuming you want to link the full name with a WordPress user ID

    global $wpdb;
    $customer_table = $wpdb->prefix . 'wp2023_customers';

    // Check if a session already exists with this full name
    if (wp2023_check_active_session($full_name)) {
        // Handle the case where an active session exists
        wp_send_json_error('A user with this name is already online.');
        wp_die();
    }

    // Proceed with saving the full name if no active session is found
    // Check if this user already has a record in the custom table
    $exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $customer_table WHERE UserId = %d", $user_id));
