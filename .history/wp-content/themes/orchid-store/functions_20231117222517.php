...
function get_template_for_product_category($product_id) {
    $terms = wp_get_post_terms($product_id, 'product_cat');
    if ($terms && !is_wp_error($terms)) {
        $category_id = $terms[0]->term_id;
        
        global $wpdb;
        $table_name = $wpdb->prefix . 'wp2023_templates';
        $template = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM $table_name WHERE CategoryId = %d",
            $category_id
        ));
...
add_action('woocommerce_before_single_product', 'display_custom_image_uploader');

function display_custom_image_uploader() {
    global $product;
    $category = 'Khung để bàn'; // The slug of the category

    // Check if the product is in the 'Khung để bàn' category
    if (has_term($category, 'product_cat', $product->get_id())) {
        // Get the template for 'Khung để bàn'
        $template = get_template_for_product_category($product->get_id());

        if ($template) {
            echo"<div id="image-editor">"
    // <!-- Input for image upload -->
    		echo"<input type="file" id="custom-image" accept="image">"
    // <!-- Image element where the uploaded image will be displayed -->
    		echo"<img id="image" style="display:none;">"
			echo"</div>";
            ?>
<div id="custom-image-uploader">
    <!-- Your HTML and JS for the image uploader and editor -->
    <input type="file" id="custom-image" accept="image/*" />
    <canvas id="image-canvas"></canvas>
    <!-- Other necessary HTML elements -->
</div>

<!-- Enqueue your image editing library and initialize it -->
<script>
// JavaScript to handle image uploading and editing
</script>
<?php
        }
    }
...
function wp2023_enqueue_image_editor_assets() {
    wp_enqueue_style('cropper-css', 'path_to_cropper.css');
    wp_enqueue_script('cropper-js', 'path_to_cropper.js', array('jquery'), '', true);
}
add_action('wp_enqueue_scripts', 'wp2023_enqueue_image_editor_assets');