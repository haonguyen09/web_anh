...
function handle_original_image_upload() {
 ...
 // Save full name to customers table and get the customer ID
    ...
    foreach ( (array) $_FILES['wp2023-custom-image']['name'] as $key => $value ) {
        if ( $_FILES['wp2023-custom-image']['error'][ $key ] == 0 ) {
            $file = array(
                'name'     => $_FILES['wp2023-custom-image']['name'][ $key ],
                'type'     => $_FILES['wp2023-custom-image']['type'][ $key ],
                'tmp_name' => $_FILES['wp2023-custom-image']['tmp_name'][ $key ],
                'error'    => $_FILES['wp2023-custom-image']['error'][ $key ],
                'size'     => $_FILES['wp2023-custom-image']['size'][ $key ]
            );
