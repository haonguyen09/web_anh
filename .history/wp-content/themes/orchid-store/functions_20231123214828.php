...
<div id="wp2023-tabletop-frame-editor">
    ...
    <div id="upload-container">
        ...
        <button id="submit-button">MUA HÀNG</button>
    </div>
    ...
    <button id="buy" data-shopee-link="<?php echo esc_attr($shopee_link); ?>">MUA HÀNG</button>
...
jQuery(document).ready(function($) {
    ...
    $('#buy').on('click', function() {
        var fullName = $('#userFullName').val().trim();

        // Check if the full name is not empty
        if (fullName === '') {
            $('#fullnameError').text('Please enter your full name.').show();
            return false;
        }

        // Save the full name first
        saveCustomerFullName(fullName, function(response) {
            if (response.success) {
                // Use the returned customer ID to save the images
                var customerId = response.data.customerId;
                uploadImagesToServer(customerId, function(success) {
                    if (success) {
                        // If images are saved successfully, redirect to Shopee
                        redirectToShopee();
                    } else {
                        // Handle error in saving images
                        alert('There was an error saving the images.');
                    }
                });
            } else {
                // Handle error in saving full name
                alert('There was an error saving your full name.');
            }
        });
    });

    function saveCustomerFullName(fullName, callback) {
        $.ajax({
            url: <?php echo admin_url('admin-ajax.php'); ?>,
            type: 'POST',
            data: {
                'action': 'save_fullname',
                'fullname': fullName,
                'security': wp2023_ajax_object.nonce
            },
            success: function(response) {
                callback(response); // Pass the entire response object to the callback
            },
            error: function() {
                callback({
                    success: false
                }); // Pass an error indication to the callback
            }
        });
    }

    function uploadImagesToServer(customerId, callback) {
        ...
        $.ajax({
            url: wp2023_ajax_object.ajax_url,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    function redirectToShopee() {
        var shopeeLink = document.getElementById('buy').getAttribute('data-shopee-link');
        // Redirect to the Shopee link
        if (shopeeLink) {
            window.location.href = shopeeLink;
        } else {
            alert('Shopee link is not set.');
        }
    }
