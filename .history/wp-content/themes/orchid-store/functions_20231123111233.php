...
function custom_add_to_cart_action($cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data) {
    // Assuming you have a function to handle the saving of images and customer data
    save_product_data_to_database($product_id, $quantity);

    // Get the Shopee link
    $shopee_link = get_post_meta($product_id, 'link_shopee', true);

    if (!empty($shopee_link)) {
        // Perform a redirect to the Shopee link
        wp_redirect($shopee_link);
        exit;
    }
}


function save_product_data_to_database($product_id, $quantity) {
    global $wpdb;

    // Implement your logic to save the necessary data
    // This might involve saving image data, customer details, etc.
    // Utilize $wpdb for any database operations
}