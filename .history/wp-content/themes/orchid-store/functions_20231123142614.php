...
jQuery(document).ready(function($) {
    ...
    $('#buy').on('click', function() {
        ...
        var data = {
            'action': 'save_fullname', // This should be the same as the PHP function hooked to 'wp_ajax_'
            'fullname': fullName,
            'security': wp2023_ajax_object.nonce // Assuming you have localized this nonce as before
        };
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    alert('Full name saved successfully');
                    // Additional success logic here
                } else {
                    $('#fullnameError').text(response.data.message).show();
                }
            },
            error: function() {
                $('#fullnameError').text(
                    'An error occurred while submitting your full name.').show();
            }
        });
    });
    ...
    // Trigger when #buy button is clicked
    $('#buy').on('click', function() {
        uploadImagesToServer(filesToUpload); // Call the upload function with the stored files
    });
    ...
    function uploadImagesToServer() {
        ...
        $.ajax({
            url: '<?php echo admin_url('admin-ajax.php'); ?>', // Replace with the path to your PHP upload handler
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
