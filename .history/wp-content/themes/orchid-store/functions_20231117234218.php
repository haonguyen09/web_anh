...
<div id="wp2023-tabletop-frame-editor">
    ...
    <canvas id="wp2023-image-canvas" style="display:none;"></canvas>
    <!-- Dropdown for standard sizes -->
    <select id="wp2023-standard-sizes">
        <option value="10x15">10x15 cm</option>
        <option value="13x18">13x18 cm</option>
        <option value="15x20">15x20 cm</option>
    </select>
    <button id="wp2023-upload-button">Upload and Edit</button>
</div>

<!-- JavaScript to handle image upload and editing -->
<script>
jQuery(document).ready(function($) {
    var cropper;
    var $image = $('#wp2023-image-canvas');
    ...
    // Add additional event listeners as needed, e.g., for cropping action
    $('#wp2023-upload-button').on('click', function() {
        if (cropper) {
            var croppedCanvas = cropper.getCroppedCanvas();
            // You can now use croppedCanvas.toDataURL() to get the image data
            // or croppedCanvas.toBlob() to get the image blob for uploading
        }
    });
});
</script>
