...
function wp2023_save_full_name() {
    ...
    // Sanitize the full name received from AJAX request
    $full_name = sanitize_text_field($_POST['full_name']);

    // Assuming you're using the logged-in user's ID, if not, adjust accordingly
    $user_id = get_current_user_id();

    global $wpdb;
    $customer_table = $wpdb->prefix . 'wp2023_customers';

    // First, check if a user with this full name is already online
    if (wp2023_check_active_session($full_name)) {
        // Return an error message if a session with this full name exists
        wp_send_json_error('A user with this name is already online.');
        wp_die();
    }

    // Check if there's an existing customer record for the current user
    $customer_exists = $wpdb->get_var($wpdb->prepare(
        "SELECT COUNT(*) FROM $customer_table WHERE UserId = %d",
        $user_id
    ));
