...
function get_the_current_category_id() {
    ...
    if ( is_a( $product, 'WC_Product' ) ) {
        ...
        if ( ! empty( $categories ) && ! is_wp_error( $categories ) ) {
            // Debugging: Output category IDs
            error_log('Product categories: ' . implode(', ', wp_list_pluck($categories, 'term_id')));
            
            return $categories[0]->term_id;
        } else {
            // Debugging: Output error if there is one
            if ( is_wp_error( $categories ) ) {
                error_log('Error retrieving product categories: ' . $categories->get_error_message());
            }
        }
    }

    // Debugging: Output a message if no product is found
    error_log('No global product found or no categories assigned.');
    return 0;
}
