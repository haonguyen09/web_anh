...
function wp2023_enqueue_scripts() {
    if (is_page('khung_de_ban') || is_singular('product')) {
        ...
        wp_enqueue_script('wp2023-admin-script', get_template_directory_uri() . '/js/admin-script.js', array('jquery'), '1.0.0', true);
    }
}
add_action('wp_enqueue_scripts', 'wp2023_enqueue_scripts');

function wp2023_print_scripts() {
    // Fetch the maximum number of image uploads setting from WordPress options
    $max_images = get_option('wp2023_max_images', 10); // Default to 10 if the setting is not found
    
    // Print the JavaScript code with the setting value
    echo "<script type='text/javascript'>
        var wp2023_settings = {
            maxUploads: " . esc_js($max_images) . "
        };
    </script>";
}
add_action('wp_head', 'wp2023_print_scripts');
...
function wp2023_settings_init() {
    // Register a new setting for WP2023 to store the maximum number of images
    register_setting('wp2023_options', 'wp2023_max_images');
    register_setting('wp2023_options', 'wp2023_image_deletion_days'); // Register a new setting for deletion days

    // Add a new section to WP2023 settings
    ...
    // Add a new field for maximum number of images
    add_settings_field(
        'wp2023_max_images',
        'Max Images Upload',
        'wp2023_max_images_field_cb',
        'wp2023-settings',
        'wp2023_settings_section'
    );
    
    // Add a new field for image deletion days
    add_settings_field(
        'wp2023_image_deletion_days',
        'Image Deletion Days',
        'wp2023_image_deletion_days_field_cb',
        'wp2023-settings',
        'wp2023_settings_section'
    );
}
...
function wp2023_image_deletion_days_field_cb() {
    $deletion_days = get_option('wp2023_image_deletion_days');
    ?>
<input type="number" name="wp2023_image_deletion_days"
    value="<?= isset($deletion_days) ? esc_attr($deletion_days) : ''; ?>" min="0">
<?php
}
...
jQuery(document).ready(function($) {
    ...
    function validateFiles(files) {
        // Reset validFiles only when new files are chosen
        validFiles = [];
        ...
        Array.from(files).forEach(file => {
            ...
            // If the file passed both checks, add it to the valid files array
            validFiles.push(file);
            readAndPreviewImage(file, validFiles.length - 1);
        });

        if (validFiles.length === 0) {
            displayError('No valid files to upload. Please check your files and try again.');
        }
    }
    ...
    $('#wp2023-custom-image').on('change', function(event) {
        ...
        var maxUploads = wp2023_settings.maxUploads || 10;
    ...
    $('#image-upload-form').on('submit', function(e) {
        ...
        // Check if there are valid files after validation
        if (validFiles.length === 0) {
            alert('No valid files to upload. Please check your files and try again.');
            return; // Do not proceed with AJAX if no valid files
        }

        var formData = new FormData(this);
        formData.append('action', 'handle_original_image_upload');
        formData.append('fullname', jQuery('#userFullName').val());
        ...
        $.ajax({
            url: '<?php echo admin_url('admin-ajax.php'); ?>', // Replace with actual AJAX URL
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    function uploadCroppedImage(blob, editType, parameter) {
        ...
        formData.append('action', 'handle_edited_image_upload');
        formData.append('imageId', currentImageIndex);
        ...
        $.ajax({
            ...
            contentType: false,
            success: function(response) {
                console.log('Edit image upload success', response);
            },
            error: function(xhr, status, error) {
                console.log('Edit image upload error', error);
            }
        });
    }

    function closeModalAndUpdateThumbnail(EditCanvas, currentImageIndex) {
        // Close the modal
        var modal = $('#image-edit-modal');
        modal.hide();
        if (currentCropper) {
        ...
        var thumbnail = $("#wp2023-image-thumbnails-container .thumbnail[data-index='" + currentImageIndex +
            "']");

        if (thumbnail.length) {
            // If the thumbnail exists, update its source
            thumbnail.attr('src', thumbnailUrl);
        } else {
            // If the thumbnail doesn't exist, append a new one
            $('#wp2023-image-thumbnails-container').append(
                $('<img>', {
                    'src': thumbnailUrl,
                    'class': 'thumbnail',
                    'data-index': currentImageIndex, // Make sure to maintain the data-index
    ...
    function displayError(errorMessage) {
        ...
        if (errorContainer.length) {
            ...
            // If the container doesn't exist, create it or log the error to the console
...
function handle_original_image_upload() {
 ...
 // Save full name to customers table and get the customer ID
    ...
    $uploadedfile  = $_FILES['wp2023-custom-image'];
    ...
    if ($movefile && !isset($movefile['error'])) {
        ...
        if ($inserted) {
            $upload_success[] = ['url' => $movefile['url']];
        } else {
            error_log('Failed to insert image: ' . $wpdb->last_error);
            wp_send_json_error(['message' => 'Failed to upload file', 'error' => $wpdb->last_error]);
            wp_die();
        }
    
    } else {
        // Handle individual file upload error
        wp_send_json_error(array('message' => 'Failed to upload file', 'error' => $movefile['error']));
        wp_die();
    }

     // Return success response if files were uploaded
    if (!empty($upload_success)) {
        wp_send_json_success(array('message' => 'Full name and images uploaded successfully', 'uploaded_files' => $upload_success));
    } else {
...
function handle_edited_image_upload() {
    ...
    // Make sure this handler is being called with a specific image to update
    $imageId = isset($_POST['imageId']) ? intval($_POST['imageId']) : 0;

    // Check if the image ID is provided and is valid
    if (!$imageId) {
        wp_send_json_error(['message' => 'Missing or invalid image ID.']);
        wp_die();
    }
    ...
    if ($movefile && !isset($movefile['error'])) {
        // Get edit type and parameters from the request
        $edit_type = isset($_POST['editType']) ? sanitize_text_field($_POST['editType']) : '';
        $parameters = isset($_POST['parameters']) ? sanitize_text_field($_POST['parameters']) : '';

        // Insert into database
        $updated = $wpdb->update(
            "{$wpdb->prefix}wp2023_images",
            array(
                'EditedImagePath' => $movefile['url'], 
                'UploadDate' => current_time('mysql'),
                'Edit_type' => $edit_type,
                'Parameters' => $parameters,
                // Add other fields as necessary, e.g., CustomerId, ProductId
            ),
            array('ImageId' => $imageId), // Where clause to identify the record to update
        ...
        if ($updated) {
            ...
            wp_send_json_error(array('message' => 'Failed to update the database record', 'error' => $wpdb->last_error));
        }
    } else {
...
function wp2023_delete_old_images() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'wp2023_images';
    $deletion_days = get_option('wp2023_image_deletion_days', 10); // Default to 30 days if not set

    $wpdb->query(
        $wpdb->prepare(
            "DELETE FROM {$table_name} WHERE DATEDIFF(CURDATE(), UploadDate) > %d",
            $deletion_days
        )
    );
}

// Schedule the image deletion function, if not already scheduled
if (!wp_next_scheduled('wp2023_daily_image_cleanup')) {
    wp_schedule_event(time(), 'daily', 'wp2023_daily_image_cleanup');
}

add_action('wp2023_daily_image_cleanup', 'wp2023_delete_old_images');