...
function wp2023_enqueue_scripts() {
    if (is_page('khung_de_ban') || is_singular('product')) {
        ...
        wp_localize_script('wp2023-custom-script', 'wp2023_ajax_object', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'max_images' => get_option('wp2023_max_images', '10') // Default to 10 if not set
        ));
    }
}
...
jQuery(document).ready(function($) {
    ...
    function readAndPreviewImage(file, index) {
        ...
        reader.onload = function(e) {
            let thumbnailContainer = $(
                '<div class="thumbnail-container" style="margin-bottom: 10px;"></div>');

            let thumbnail = $(
                '<img class="thumbnail" style="max-width: 100px; height: 100px; cursor: pointer; display: inline-block;">'
            );
            thumbnail.attr('src', e.target.result);
            thumbnail.attr('data-index', index);

            let fileInfo = $(
                '<div style="display: inline-block; margin-left: 10px; vertical-align: top;"></div>');
            fileInfo.append(`<p>Name: ${file.name}</p>`);
            fileInfo.append(`<p>Size: ${Math.round(file.size / 1024)} KB</p>`);

            thumbnailContainer.append(thumbnail);
            thumbnailContainer.append(fileInfo);
    ...
    var maxUploads = parseInt(wp2023_max_images);
