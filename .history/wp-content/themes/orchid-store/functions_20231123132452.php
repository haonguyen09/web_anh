...
function wp2023_redirect_to_shopee() {
    if (is_product()) {
        global $post;
        $shopee_link = get_post_meta($post->ID, 'link_shopee', true);
        if (!empty($shopee_link)) {
            wp_redirect(esc_url($shopee_link));
            exit;
        }
    }
}
add_action('woocommerce_before_add_to_cart_button', 'wp2023_redirect_to_shopee');