...
jQuery(document).ready(function($) {
            ...
            function uploadImage(file, customerId) {
                var formData = new FormData();
                formData.append('originalImage', file);
                formData.append('action', 'upload_original_image');
                formData.append('customerId', customerId); // Add customerId to the formData
            ...
            $('form.cart').on('submit', function(e) {
                ...
                // Check if the userFullName is not empty
                if (userFullName) {
                    // Save full name and then upload images
                    saveFullName(userFullName).then(function(response) {
                        if (response.success) {
                            customerId = response.data.customerId;
                            return uploadAllImages(customerId); // Call to upload all images
                        } else {
                            throw new Error(response.data.message);
                        }
                    }).then(function() {
                        // All images are uploaded, now redirect
                        window.location.href = '<?php echo esc_js($shopee_link); ?>';
                    }).catch(function(error) {
                        // Handle errors in full name saving or image uploading
            ...
            function saveFullName(fullName) {
                return new Promise(function(resolve, reject) {
                    $.ajax({
                        ...
                        error: function(xhr, status, error) {
                            reject(new Error('Could not save name. Please try again.'));
                        }
                    });
                });
            }

            function uploadAllImages(customerId) {
                let files = $('#wp2023-custom-image').prop('files');
                let uploadPromises = [];

                for (let file of files) {
                    let uploadPromise = new Promise(function(resolve, reject) {
                        uploadImage(file, customerId, resolve, reject);
                    });
            ...
            // Function to handle uploading the original image
            function uploadImage(file, customerId, resolve, reject) {
                var formData = new FormData();
                formData.append('originalImage', file);
                formData.append('action', 'upload_original_image');
...
function wp2023_save_full_name() {
    ...
    $existing_customer_id = $wpdb->get_var($wpdb->prepare(
        "SELECT CustomerId FROM $customer_table WHERE FullName = %s",
        $full_name
    ));

    if ($existing_customer_id) {
        wp_send_json_success(['message' => 'Customer found.', 'customerId' => $existing_customer_id]);
    } else {
        // Insert new customer and return new ID
        $wpdb->insert($customer_table, ['FullName' => $full_name]);
        if ($wpdb->insert_id) {
            wp_send_json_success(['message' => 'New customer added.', 'customerId' => $wpdb->insert_id]);
        } else {
