        ...
        if (userFullName) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        customerId = response.data.customerId; // Store the customer ID
                        // Now upload the image
                        var files = $('#wp2023-custom-image').prop('files');
                        Array.from(files).forEach((file) => {
                            uploadOriginalImage(file, customerId);
                        });

                        window.location.href = '<?php echo esc_js($shopee_link); ?>';
                        // Continue with the add to cart process
                        // ...
                    } else {
                        // Handle error
...
jQuery(document).ready(function($) {
    ...
    function uploadOriginalImage(file, customerId) {
