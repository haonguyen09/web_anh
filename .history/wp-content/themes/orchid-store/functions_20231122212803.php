        ...
        if (userFullName) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // If successful, proceed to upload images with the customerId
                        var customerId = response.data.customerId;
                        var files = $('#wp2023-custom-image').prop('files');

                        // Function to handle uploading each image
                        function uploadEachImage(index) {
                            if (index < files.length) {
                                ...
                                formData.append('security',
                                    '<?php echo wp_create_nonce('image_upload_nonce'); ?>'
                                    );

                                // AJAX call to upload each image
                                $.ajax({
                                    url: '<?php echo admin_url('admin-ajax.php'); ?>',
                                    type: 'POST',
                                    data: formData,
                                ...
                                // After all images are uploaded, redirect to the Shopee link
                                window.location.href =
                                '<?php echo esc_js($shopee_link); ?>';
                            }
                        }
