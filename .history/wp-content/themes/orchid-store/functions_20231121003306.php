...
jQuery(document).ready(function($) {
    ...
    $('#wp2023-apply-crop').on('click', function() {
        if (currentCropper) {
            ...
            croppedCanvas.toBlob(function(blob) {
                ...
                closeModalAndUpdateThumbnail(croppedCanvas);
            }, 'image/png');
        }
    });
    ...
    function uploadCroppedImage(blob) {
        ...
        formData.append('lastEditType', editType);
        formData.append('lastParameter', parameter); // The WordPress action for edited image
    ...
    // Function to close the modal and update the thumbnail
    function closeModalAndUpdateThumbnail(croppedCanvas) {
        // Close the modal
        var modal = $('#image-edit-modal');
        modal.hide();
        if (currentCropper) {
        ...
        // Assuming you want to update the last clicked thumbnail or append a new one
        var lastThumbnail = $('#wp2023-image-thumbnails-container .thumbnail').last();
        if (lastThumbnail.length > 0) {
            lastThumbnail.attr('src', thumbnailUrl);
        } else {
            // If no thumbnail exists, create a new one
