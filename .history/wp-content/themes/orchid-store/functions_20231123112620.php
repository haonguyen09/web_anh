...
jQuery(document).ready(function($) {
    ...
    function uploadImagesToServer() {
        ...
        $.ajax({
            url: '/wp-admin/admin-ajax.php', // Replace with the path to your PHP upload handler
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
...
function handle_original_image_upload() {
    ...
    if (!isset($_POST['customerId']) || !is_numeric($_POST['customerId'])) {
        wp_send_json_error(['message' => 'Invalid or missing customer ID']);
        wp_die();
    }
    ...
    if (empty($_FILES['originalImage']['name'])) {
    ...
    if ($movefile && !isset($movefile['error'])) {
        ...
        $wpdb->insert(
            ...
            array(
                ...
                'Edit_type' => null,
                'Parameters' => null,
                'CustomerId' => $customerId, // Example: get the current logged-in user ID
                'ProductId' => get_the_ID(), // Example: get the current product ID if on a single product page
                'TemplateId' => 0, // Example: set this appropriately based on your application logic
            ),
...
function capture_additional_product_fields( $cart_item_data, $product_id, $variation_id ) {
    if( isset( $_POST['full_name_field'] ) ) {
        $cart_item_data['full_name'] = sanitize_text_field( $_POST['full_name_field'] );
    }
    return $cart_item_data;
}
add_filter( 'woocommerce_add_cart_item_data', 'capture_additional_product_fields', 10, 3 );

function save_full_name_to_custom_table( $item_id, $values ) {
    if ( isset( $values['full_name'] ) ) {
        global $wpdb;
        $wpdb->insert(
            "{$wpdb->prefix}wp2023_customers",
            array( 'FullName' => $values['full_name'] ),
            array( '%s' )
        );
    }
}
add_action( 'woocommerce_new_order_item', 'save_full_name_to_custom_table', 10, 2 );

function redirect_to_shopee_link( $url ) {
    global $woocommerce;
    foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
        $product_id = $cart_item['product_id'];
        // Assuming you've stored the Shopee link as a custom field in the product meta
        $shopee_link = get_post_meta( $product_id, 'link_shopee', true );
...
add_filter( 'woocommerce_add_to_cart_redirect', 'redirect_to_shopee_link' );

jQuery(document).ready(function($) {
    $('body').on('submit', 'form.cart', function(e) {
        e.preventDefault();

        var formData = $(this).serializeArray(); // This captures all form data including the full name
        formData.push({name: 'full_name_field', value: $('#full_name_input_field_id').val()}); // Replace with your full name field ID
        ...
        $.ajax({
            ...
            success: function(response) {
                // Handle the response
                // Maybe check for a certain condition in the response to decide whether to redirect
            },
