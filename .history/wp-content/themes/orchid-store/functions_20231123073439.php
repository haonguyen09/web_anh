...
function wp2023_handle_add_to_cart() {

    global $wpdb;
    // Use AJAX to handle this request
    check_ajax_referer('add_to_cart_nonce', 'security');

    // Get user full name and validate it
    ...
    if (empty($fullName)) {
     ...
     // Get Shopee link from product meta
    $productId = get_the_ID();
    $shopeeLink = get_post_meta($productId, 'link_shopee', true);

    if (empty($shopeeLink)) {
        wp_send_json_error(['message' => 'Shopee link is missing for this product']);
        wp_die();
    }

     // Assuming the image IDs are passed in the request
    $originalImageId = isset($_POST['originalImageId']) ? intval($_POST['originalImageId']) : 0;
    $editedImageId = isset($_POST['editedImageId']) ? intval($_POST['editedImageId']) : 0;

    // Validate image IDs
    if ($originalImageId <= 0 || $editedImageId <= 0) {
        wp_send_json_error(['message' => 'Invalid image IDs']);
        wp_die();
    }
...
add_action('wp_ajax_wp2023_handle_add_to_cart', 'wp2023_handle_add_to_cart');
add_action('wp_ajax_nopriv_wp2023_handle_add_to_cart', 'wp2023_handle_add_to_cart');
