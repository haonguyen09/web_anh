...
function wp2023_custom_add_to_cart_button($button, $product) {
    ...
    if (empty($shopee_link)) {
        return $button; // If no Shopee link is set, return the default button
    }
    
    // Create a new button that links to the Shopee page
    $button_text = __('MUA HÀNG', 'wp-photo'); // Your button text
    $button_link = esc_url($shopee_link);
    $button = '<a class="button" href="' . $button_link . '">' . $button_text . '</a>';
    
    return $button;
}

add_action('woocommerce_process_product_meta', 'wp2023_save_shopee_link_meta', 10, 1);
function wp2023_save_shopee_link_meta($post_id) {
    if (isset($_POST['wp2023_shopee_link'])) {
        update_post_meta($post_id, 'wp2023_shopee_link', sanitize_text_field($_POST['wp2023_shopee_link']));
    }
}