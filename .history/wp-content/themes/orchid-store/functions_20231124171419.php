...
function wp2023_enqueue_scripts() {
    if (is_page('khung_de_ban') || is_singular('product')) {
        ...
        $max_images = get_option('wp2023_max_images');

        // Prepare the settings array to pass to JavaScript
        $settings = array(
            'maxUploads' => $max_images ? $max_images : 10, // Provide a default value if the setting isn't set
        );
    // Localize the script with your settings
    wp_localize_script('wp2023-script', 'wp2023_settings', $settings);
...
jQuery(document).ready(function($) {
    ...
    function validateFiles(files) {
        ...
        validFiles = []; // Reset the valid files array
    ...
    $(document).on('change', '#wp2023-custom-image', function(event) {
        var files = event.target.files;

        if (files.length > (wp2023_settings.maxUploads || 10)) {
            displayError(`You can only upload up to ${wp2023_settings.maxUploads || 10} images.`);
            return;
        }
    ...
    $('#image-upload-form').on('submit', function(e) {
        ...
        if (validFiles.length === 0) {
            ...
            return; // Stop form submission if no valid files
        }
    ...
    function displayError(errorMessage) {
        ...
        if (errorContainer.length) {
            ...
            // Alternatively, you can create the container dynamically:
            // $('<div id="error-message-container"></div>').text(errorMessage).appendTo('body').show();
        }
    }
