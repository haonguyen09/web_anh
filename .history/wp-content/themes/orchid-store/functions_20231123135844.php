...
// Redirect to Shopee when a product is added to the cart
function wp2023_redirect_to_shopee($cart_item_key, $product_id) {
    $shopee_link = get_post_meta($product_id, 'link_shopee', true);
    if (!empty($shopee_link)) {
        wp_safe_redirect($shopee_link);
        exit;
    }
}
add_action('woocommerce_add_to_cart', 'wp2023_redirect_to_shopee', 10, 2);