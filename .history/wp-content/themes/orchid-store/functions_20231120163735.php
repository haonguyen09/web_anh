...
function handle_image_upload() {
    ...
    // Check if both original and edited images are set
    if (isset($_FILES['originalImage'], $_FILES['editedImage'])) {
        // Process the original image
        $originalFile = $_FILES['originalImage'];
        $upload_dir = wp_upload_dir();
        $originalPath = $upload_dir['path'] . '/' . $originalFile['name'];
        move_uploaded_file($originalFile['tmp_name'], $originalPath);

        // Process the edited image
        $editedFile = $_FILES['editedImage'];
        $editedPath = $upload_dir['path'] . '/edited_' . $editedFile['name'];
        move_uploaded_file($editedFile['tmp_name'], $editedPath);
        ...
        $wpdb->insert(
            ...
            array(
                ...
                'UploadDate' => current_time('mysql'), // Assuming you want to store the upload date
                // Other fields as necessary...
            )
        );

        // Check for successful insertion and respond accordingly
        if ($wpdb->insert_id) {
            wp_send_json_success('Image uploaded and saved successfully');
        } else {
            wp_send_json_error('Error in storing image data');
        }
    } else {
