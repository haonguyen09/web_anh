...
jQuery(document).ready(function($) {
    ...
    //     if (cropperData) {
    //         currentCropper = cropperData.currentCropper;
    //     }
    // });


    // When a file is chosen, read it and initialize the cropper
    $('#wp2023-custom-image').on('change', function(event) {
        if (event.target.files.length) {
            ...
            reader.onload = function(e) {
                ...
                imageToEdit.attr('src', e.target.result);
                modal.show();
                initializeCropper(imageToEdit[0]); // Initialize Cropper.js on the new image
            };
            reader.readAsDataURL(file);
        }
    });
    ...
    $('#modal-apply-crop').on('click', function() {
        ...
        croppedCanvas.toBlob(function(blob) {
            ...
            formData.append('originalImage', $('#wp2023-custom-image').prop('files')[
                currentImageIndex]);
            formData.append('editType', 'Crop'); // Example edit type
            formData.append('parameters', 'Your parameters here');

            $.ajax({
                url: '/wp-admin/admin-ajax.php', // Modify this path
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
...
function handle_image_upload() {
    ...
    if (isset($_FILES['originalImage']) && isset($_FILES['editedImage'])) {
        $originalFiles = $_FILES['originalImage'];
        $editedFiles = $_FILES['editedImage'];
        $num_files = count($originalFiles['name']);

        // Verify that the wp_wp2023_images table exists
        if ($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}wp2023_images'") != "{$wpdb->prefix}wp2023_images") {
            wp_send_json_error('Database table does not exist.');
            wp_die();
        }

        for ($i = 0; $i < $num_files; $i++) {
            // Skip file if not uploaded
            if ($originalFiles['error'][$i] != 0 || $editedFiles['error'][$i] != 0) {
                wp_send_json_error('Error in file upload for image index ' . $i);
                continue;
            }

            // Process the original image
            $upload_dir = wp_upload_dir();
            $originalPath = $upload_dir['path'] . '/' . $originalFiles['name'][$i];
            if (!move_uploaded_file($originalFiles['tmp_name'][$i], $originalPath)) {
                wp_send_json_error('Failed to move original file for image index ' . $i);
                continue;
            }

            // Process the edited image
            $editedPath = $upload_dir['path'] . '/edited_' . $editedFiles['name'][$i];
            if (!move_uploaded_file($editedFiles['tmp_name'][$i], $editedPath)) {
                wp_send_json_error('Failed to move edited file for image index ' . $i);
                continue;
            }

            // Additional fields from the request
            $editType = sanitize_text_field($_POST['editType'][$i]);
            $parameters = sanitize_text_field($_POST['parameters'][$i]);
            ...
            // Handle possible insert error
            if (!$result) {
                wp_send_json_error('Error in storing data for image index ' . $i . '. Error: ' . $wpdb->last_error);
            }
        }

        wp_send_json_success('All images processed and saved successfully');
    } else {
