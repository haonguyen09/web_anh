...
// CHANGE SINGLE PAGE PRODUCT

//Change action Add to cart to MUA HÀNG button
...
function wp2023_custom_cart_button_text() {
    return __('MUA HÀNG', 'wp-photo'); // Change 'wp-photo' to your theme or plugin text domain.
}
...
jQuery(document).ready(function($) {
    $('form.cart').on('submit', function(e) {
        ...
        window.location.href = '<?php echo esc_js($shopee_link); ?>'; // Redirect to Shopee link
    });
});
</script>
<?php
...
// Change the "Add to Cart" button text on archive pages
add_filter('woocommerce_product_add_to_cart_text', 'wp2023_change_add_to_cart_button_text', 10, 2);
function wp2023_change_add_to_cart_button_text($text, $product) {
    return __('MUA HÀNG', 'wp-photo'); // Your button text and text domain
}

// Change the "Add to Cart" button on archive pages to a link
...
function wp2023_custom_add_to_cart_button($button, $product) {
    // Get the Shopee link from the product meta
    $shopee_link = get_post_meta($product->get_id(), 'wp2023_shopee_link', true);
    if (!$shopee_link) {
        // If there's no Shopee link, return the default button
        return $button;
    }

    // Return a custom button with the Shopee link
    return '<a href="' . esc_url($shopee_link) . '" class="button product_type_simple add_to_cart_button ajax_add_to_cart">' . __('MUA HÀNG', 'wp-photo') . '</a>';
}