        ...
        $table_definitions = [
            'wp2023_customers' => "
                CREATE TABLE {$wpdb->prefix}wp2023_customers (
                    ...
                    FOREIGN KEY (UserId) REFERENCES {$wpdb->prefix}users(ID) ON DELETE CASCADE
                ) $charset_collate;",
            ...
            'wp2023_images' => "
                CREATE TABLE {$wpdb->prefix}wp2023_images (
                    ...
                    PRIMARY KEY (ImageId),
                    FOREIGN KEY (CustomerId) REFERENCES {$wpdb->prefix}wp2023_customers(CustomerId) ON DELETE CASCADE,
                    FOREIGN KEY (WooCommerceProductId) REFERENCES {$wpdb->prefix}posts(ID) ON DELETE CASCADE,
                    FOREIGN KEY (TemplateId) REFERENCES {$wpdb->prefix}wp2023_templates(TemplateId) ON DELETE CASCADE
                ) $charset_collate;"
        ];
    
        foreach ($table_definitions as $table_sql) {
            dbDelta($table_sql);
        }
    // }
}
